 ...
 * @type {Dictionary}
 */
module.exports = (function sailsCassandra(){
  ...
  registerDatastore: function (datastoreConfig, models, done) {
    ...
    if (!identity) {
      return done(new Error('Invalid datastore config. A datastore should contain a unique identity property.'));
    }

    //Register datastore using his identity, config and models with our helper modules.
  ...
  teardown: function (datastoreName, done) {
    ...
    var dsEntry = datastores[datastoreName];
  ...
  create: function (datastoreName, query, done) {
    ...
    if(_.isUndefined(models)) {
      return done(new Error('Consistency violation: Cannot do that with datastore (`'+datastoreName+'`) because no matching models are registered in this adapter!'));
    }
  ...
  join: function (datastoreName, query, done) {
    // not supported
    return done(new Error('Unfortunately, join operations are not supported by Cassandra.'));
  },
